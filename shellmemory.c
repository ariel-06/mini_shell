...
*/
    ...
    int pid = 0;
    for (int i = 0; i < 10; i++){          // index i represents an available pid
    ...
    //otherwise finds next available slot in the line array to store the first line of the script
    int start = 0;
    while(start < 1000 && script_memory[start].owner != 0)
        //owner = 0 when index is not a line associated with a script
        start++;
    ...
    pcb->start_index = start;      //this is the empty index in the line array we found
    pcb->length = 0;               // initial length is 0; this will be updated after all lines are stored
    ...
    while (fgets(buffer, 100, file) != NULL){
        ...
        //otherwise store the line from the file into memory
        ...
        //do this again until EOF or until memory runs out and function fails 
        i++;
    }
